 ...
 */
 Mat get_foreground(Mat image){
  Mat result,bgModel,fgModel,bigger_img=Mat::zeros(96,96,CV_32F),ret_image=Mat::zeros(32,32,CV_32F);
  Rect rectangle(20,20,50,50);
  resize(image, bigger_img, bigger_img.size(), 0, 0, CV_INTER_CUBIC);
  grabCut(bigger_img, result, rectangle, bgModel, fgModel, 1, GC_INIT_WITH_RECT); 
  compare(result,cv::GC_PR_FGD,result,cv::CMP_EQ);
  Mat foreground(image.size(),CV_8UC3,cv::Scalar(0,0,0));
  image.copyTo(foreground,result);
  //namedWindow("foreground bicubic",1);
  //imshow("foreground bicubic",foreground);
  //waitKey(0);
  //resize(foreground, ret_image, ret_image.size(), 0, 0, CV_INTER_CUBIC);
  return Mat(foreground,Rect(32,32,32,32));
}
 ...
 */
 ...
 // pyrDown(foreground, resize_fg, Size( foreground.rows/3, foreground.cols/3) ); 
  ...
  // namedWindow("foreground bicubic",1);
  // imshow("foreground bicubic",foreground);
  // imwrite("bicubic_5.png",foreground);
